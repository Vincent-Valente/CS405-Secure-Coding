...
bool run_query(sqlite3* db, const std::string& sql, std::vector<user_record>& records) {
    ...
    // Enhanced RegEx to detect "OR value=value;" SQL injection attempts
    // This pattern is designed to capture numeric literals, quoted strings, and also considers whitespace variations
    std::regex re(R"(\bOR\b\s*('[^']*'|"[^"]*"|\d+)\s*=\s*('[^']*'|"[^"]*"|\d+)\s*;)", std::regex_constants::icase);

    // Detect SQL injection pattern match
    if (std::regex_search(sql, re)) {
        // Display an error message indicating a potential SQL injection attempt
        std::cout << "Potential SQL Injection detected (OR value=value;). Query execution blocked." << std::endl;
        return false;
    }
    ...
    if (sqlite3_exec(db, sql.c_str(), callback, &records, &error_message) != SQLITE_OK) {
        std::cout << "Error querying USERS table. ERROR = " << error_message << std::endl;
        sqlite3_free(error_message);
        return false;
    }
