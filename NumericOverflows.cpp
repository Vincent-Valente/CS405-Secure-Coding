...
void test_overflow()
{
    ...
    // First operation: Expected to complete without overflow
    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
    T result = add_numbers<T>(start, increment, steps);
    std::cout << +result << std::endl;

    // Second operation: Expected to cause overflow
    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
    T result2 = add_numbers<T>(start, increment, steps + 1);

    if (result2 - std::numeric_limits<T>::min() == result) {
        std::cout << +(result2 - std::numeric_limits<T>::min()) << std::endl;
        std::cout << "\tOverflow Ocurred" << std::endl;
    }
    else {

        std::cout << +(result2) << std::endl;
        std::cout << "\tNo Overflow Ocurred" << std::endl;
    }
